...
- A per-save **learned cap** that increases when the player obtains items with higher rollable slot counts, tracked **per (rarity, item type)** pair.
...
For each **(rarity `r`, item type `t`)** pair, the save maintains:

`OverallCap[r, t] = max(DefaultOverallCap[r], LearnedOverallCap[r, t])`

Where `LearnedOverallCap[r, t]` is the maximum **actual rollable slot count** the player has ever obtained on an item of rarity `r` and item type `t`.

**Item type granularity:**

- **Weapons**: tracked per `WeaponType` (Sword, Wand, Dagger, Axe, etc. — each tracked separately)
- **Armour**: tracked per slot (Helmet, Chest, Boots, Gloves, Pants — each tracked separately)
- **Jewellery**: tracked per type (Ring, Amulet — each tracked separately)
- **Shields**: tracked as a single category

**Note:** The default caps shown in the table above are the baseline for each rarity. Learned caps can only increase when the player actually obtains items with higher modifier counts:

- Common: default cap **0** (stays at 0 unless the player sees a Common item with modifiers)
- Uncommon: default cap **1** (increases only if the player sees an Uncommon item with more than 1 modifier)
- Rare: default cap **2** (increases only if the player sees a Rare item with more than 2 modifiers)
- Epic: default cap **3** (increases only if the player sees an Epic item with more than 3 modifiers)
- Legendary: default cap **4** (increases only if the player sees a Legendary item with more than 4 modifiers)
- Divine: default cap **5** (already at maximum)

**Update trigger (acquisition only):**

- Update the learned cap for the specific **(rarity, item type)** pair only when the player **obtains** an item (loot pickup, reward, vendor purchase).
- Do **not** update on merely viewing vendor inventory.
- Example: If the player obtains an Uncommon Ring with 2 modifiers, only `LearnedOverallCap[Uncommon, Ring]` is updated to 2. This does not affect `LearnedOverallCap[Uncommon, Helmet]` or other item types.
...
When `OverallCap[r, t]` increases, show a one-time message:

- "New stats limit for **{RarityName} {ItemTypeName}** is updated to **{OverallCap[r, t]}**. You can now forge up to **{OverallCap[r, t]}** rollable slots on **{RarityName} {ItemTypeName}** items."
