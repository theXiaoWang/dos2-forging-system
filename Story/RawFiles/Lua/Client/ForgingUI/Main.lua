...
local UI_TARGET_WIDTH_RATIO = 0.8
local UI_TARGET_HEIGHT_RATIO = 0.95
local UI_OUTER_MARGIN = 8
local UI_PANEL_MARGIN = 0
local UI_PANEL_GAP = -10
local UI_COLUMN_GAP = -10
local UI_CONTENT_INSET_X = 0
local LayoutState = {
    Width = BASE_UI_WIDTH,
    Height = BASE_UI_HEIGHT,
    ScaleX = 1,
    ScaleY = 1,
}
...
local PREVIEW_DROP_SOUND = "UI_Game_PartyFormation_PickUp"
local BASE_PANEL_ALPHA = 0.85
local SECTION_FRAME_ALPHA = 0.5
local SECTION_TEXTURE_CENTER_ALPHA = 0.2
local SECTION_FILL_ALPHA = 0.25
local TEXT_COLOR = 0xFFD700  -- Gold color (RGB: 255, 215, 0) - classic gold matching game assets
local HEADER_TEXT_SIZE = 13
local BODY_TEXT_SIZE = 11
local BORDER_COLOR = Color.CreateFromHex("2F2720")
...
local USE_BASE_BACKGROUND = false
local USE_SLICED_BASE = true
local USE_SLICED_FRAMES = true
local USE_SLICED_PANELS = true
local USE_VANILLA_COMBINE_PANEL = false
local USE_SIDE_INVENTORY_PANEL = false
local USE_CUSTOM_PREVIEW_PANEL = true
...
local PREVIEW_REFRESH_INTERVAL = 0.5
local DISPLAY_MODES = {
    Combine = "Combine",
    Preview = "Preview",
}
