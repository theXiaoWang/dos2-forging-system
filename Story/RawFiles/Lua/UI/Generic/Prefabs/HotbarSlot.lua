...
function Slot.Create(ui, id, parent, requiredFeatures)
    ...
    slot.Events.MouseOut:Subscribe(function (_)
        if Tooltip and Tooltip.HideTooltip then
            Tooltip.HideTooltip()
        end
...
function Slot:Clear()
    ...
    self:SetLabel("")
    if self.RarityIcon then
        self.RarityIcon:SetIcon("", 1, 1)
        self.RarityIcon:SetVisible(false)
    end
    if self.RuneSlotsIcon then
        self.RuneSlotsIcon:SetVisible(false)
    end
...
function Slot:_ShowTooltip()
    if not Tooltip or not GameState then
        return
    end
