...
---@param modTable string? Defaults to "forging_system"
...
function Text.GenerateLocalizationTemplate(modTable, existingTemplate)
    ...
    local template = {
        ModTable = modTable or "forging_system",
...
---@param useOverride boolean? Whether to consider the forging_system language override setting. Defaults to `true`.
---@return string
function Text.GetCurrentLanguage(useOverride)
    local language = Ext.Utils.GetGlobalSwitches().ChatLanguage
    local settingOverride = useOverride ~= false and IO.LoadFile("forging_system/LanguageOverride.txt", "user", true) or ""
